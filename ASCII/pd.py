...
## Copyright (C) 2018 Paul Fletcher <p.fletcher@digitalwigwam.com>
##
...
class Decoder(srd.Decoder):
    ...
    id = 'ascii'
    name = 'ASCII'
    longname = 'Parallel ASCII sync bus'
    desc = 'ASCII decoded parallel synchronous bus.'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['ascii']
    ...
    options = (
        ...
        {'id': 'control_chars', 'desc': 'Show control characters as',
            'default': 'names', 'values': ('names', 'values')},
        {'id': 'space_control', 'desc': 'Treat SPACE character as',
            'default': 'printable', 'values': ('printable', 'control')},
    )
    annotations = (
        ('items', 'Items'),
        ('words', 'Words'),
    ...
    def handle_bits(self, item, used_pins):
        ...
        if self.first:
            ...
            # Check to see if our Last Control Char includes the space char or not. 
            lcc = 32;
            if self.options['space_control'] == 'printable':
                lcc = 31;
            self.es_item = self.samplenum
            self.putpb(['ITEM', self.saved_item])
            if self.saved_item > 126:
                self.putb([0, [self.fmt_item.format(self.saved_item)]])
            elif self.saved_item > lcc:
                self.putb([0, [chr(self.saved_item)]])
            else:
                if self.options['control_chars'] == 'names':
                    self.putb([0, [control_chars[self.saved_item]]])
                else:
                    self.putb([0, [self.fmt_item.format(self.saved_item)]])
            self.ss_item = self.samplenum
            self.saved_item = item
